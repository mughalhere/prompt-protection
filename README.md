...
Operability (4.3): pick a preset, observe before you enforce, and read the reason chain.

```ts
createGuard({ preset: 'strict' });            // 'balanced' (default) · 'strict' · 'permissive'; explicit options win
createGuard({ mode: 'observe' });             // action is always 'allow'; decision.observedAction is the verdict
import { explain } from 'prompt-protection/guard';
explain(decision);                            // { summary, chain: [{ code, policy, because, flows }] }
```

One call per framework:

```ts
import { protect } from 'prompt-protection/adapters/vercel-guardrail';   // ai ≥ 6
const { tools, toolApproval, prepareStep, onToolExecutionEnd, stopWhen } = protect(guard, myTools);
new WorkflowAgent({ tools, toolApproval, prepareStep, onToolExecutionEnd, stopWhen }); // WorkflowAgent has no default step limit

import { protect } from 'prompt-protection/adapters/openai-agents';      // @openai/agents ≥ 0.18
const { inputGuardrails, wrapTool } = protect(guard);
new Agent({ inputGuardrails, tools: [wrapTool(sendEmail)] });           // tool guardrails + needsApproval bound per tool

import { protect, interruptWhen } from 'prompt-protection/adapters/langchain'; // langchain ≥ 1
createAgent({ middleware: [await protect(guard), humanInTheLoopMiddleware({ interruptOn: { stripe_transfer: { allowedDecisions: ['approve', 'reject'], when: interruptWhen(guard) } } })] });
```
...
| Output scan, canary variants, system-prompt similarity, credential/PII/relay rules, render-exfil (allowlist, data: URIs, data-carrying query params) | MIT | 28 (16/12) | 100% | 0% |
